...
void draw(){ //runs over and over
  ...
  //black rectanlge
  fill(tay); //color within the next shape
  rectMode(CORNER); //drawing rectangle from the center
  noStroke(); //no outline
  rect(al,al,560,330); //rect(x1,y1,x2,y2);
  
  //red quad
  stroke(255,tay,tay); //red outline color
  ...
  //left circle
  fill(tay,255,tay,150); //4th variable, transparency
  noStroke(); //no stroke
  ...
  fill(255); //fill white
  stroke(tay,tay,255); //stroke color
  ...
  //point
  stroke(255,tay,tay); //stroke color
  strokeWeight(3); //line large
